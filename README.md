...
`sb add` 创建节点时会读取 `/etc/sing-box/manager.json` 的公网地址。若该值是旧的 IPv4/IPv6 字面地址，管理器会在创建新节点前重新探测当前公网地址，并自动更新 `manager.json`；因此更换 VPS IP 后不需要重装管理器。

域名不会被自动替换：如果 `server_address` 是域名，管理器会继续使用该域名。自动探测失败时，可显式指定：

```sh
sb add ss2022 --public-address NEW_IP_OR_DOMAIN
```

旧节点的公网地址不会被自动修改，避免已发布链接被静默改变；需要更新旧节点时使用菜单中的“修改节点”。

## IPv6、双栈与 NAT IPv6

IPv4 仍是默认模式。新增节点时可选择：

- `sb ipv6`：打开 IPv6/NAT 菜单并检测地址、路由。直连模式自动使用检测到的公网 IPv6，并仅绑定该 IPv6（不占用同端口 IPv4），公网端口与内部端口相同；NAT 模式必须分别填写内部监听地址/端口与面板映射后的公网 IPv6/域名和公网端口。
